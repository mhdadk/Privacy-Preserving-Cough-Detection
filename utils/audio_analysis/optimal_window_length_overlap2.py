...
for i in range(int(num_overlap_windows/2)):
    
    start = int(i * window_length * sr)
    end = int((i + 1) * window_length * sr)
    
    full_frame = x[start:end]
    
    sub_end = int(window_length*sr)
    sub_start = sub_end - int(overlap_length*sr)
    
    subframe = full_frame[sub_start:sub_end]
    
    rms_full_frame,entropy_full_frame = get_stats(full_frame,sr)
    rms_subframe,entropy_subframe = get_stats(subframe,sr)
    
    rms_diff = rms_full_frame - rms_subframe
    entropy_diff = entropy_full_frame - entropy_subframe
    
    rms_vals[i] = rms_diff
    entropy_vals[i] = entropy_diff
