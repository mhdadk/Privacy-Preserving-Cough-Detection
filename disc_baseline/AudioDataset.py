...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,audio_type,data_split_dir,sample_rate,mode):
        
        """
        whether to sample 'other', 'cough', or 'speech' data
        """
    ...
    def _get_file_paths(self):
        ...
        if self.audio_type == 'other':
            self.paths = self.splits['other_'+self.mode]
            self.labels = [0]*len(self.splits['other_'+self.mode])
        elif self.audio_type == 'cough':
            self.paths = self.splits['cough_'+self.mode]
            self.labels = [1]*len(self.splits['cough_'+self.mode])
        else:
...
if __name__ == '__main__':
    ...
    sample_rate = 16000
    mode = 'train'
    dataset = AudioDataset(audio_type,data_split_dir,sample_rate,mode)
