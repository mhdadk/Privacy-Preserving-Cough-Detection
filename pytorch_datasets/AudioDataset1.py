...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def _get_file_paths(self):
        ...
        if self.net_type == 'disc': # discriminator network
            self.paths = (self.splits['other_short_'+self.mode] + 
                          self.splits['cough_short_'+self.mode])
            self.labels = ([0]*len(self.splits['other_short_'+self.mode]) + 
                           [1]*len(self.splits['cough_short_'+self.mode]))
        else: # reconstruction network
            self.paths = self.splits['speech_short_'+self.mode]
            self.labels = [0]*len(self.splits['speech_short_'+self.mode])
