...
mel_spec_height, tau = mel_spec.shape[1:]
device = mel_spec.device

y=mel_spec_height//2
horizontal_line_at_ctr = mel_spec[0][y]
W = 60
import random
point_to_warp = horizontal_line_at_ctr[random.randrange(W, tau - W)]

# Uniform distribution from (0,W) with chance to be up to W negative
dist_to_warp = random.randrange(-W, W)
src_pts, dest_pts = (torch.tensor([[[y, point_to_warp]]], device=device), 
                     torch.tensor([[[y, point_to_warp + dist_to_warp]]], device=device))
warped_spectro, _ = sparse_image_warp(mel_spec, src_pts, dest_pts)

mel_spec[0,:,y] = 0
warped_spectro[0,:,y+dist_to_warp,0] = 0
